...
public class CustomerOrder {
    ...
    @OneToOne(mappedBy = "order", cascade = CascadeType.ALL, fetch = FetchType.LAZY, optional = false) private OrderItem item;
    ...
    static CustomerOrder create(UUID ownerAccountPublicId, UUID branchPublicId, UUID reservationPublicId,
        ...
        order.createdAt = now;
        order.item = OrderItem.create(order, variantPublicId, locationPublicId, quantity, unitPriceAmount);
        return order;
    }
    ...
    static CustomerOrder createCheckout(UUID ownerAccountPublicId, UUID branchPublicId, UUID reservationPublicId,
        ...
        order.status = Status.PENDING_PAYMENT;
        order.createdAt = now;
        order.item = OrderItem.createCheckout(order, variantPublicId, locationPublicId, sku, size, quantity, unitPriceAmount);
        return order;
    }
    ...
    public static CustomerOrder createPos(UUID branchPublicId, UUID priceVersionId, UUID variantPublicId,
        ...
        order.createdAt = now;
        order.paidAt = now;
        order.item = OrderItem.createCheckout(order, variantPublicId, locationPublicId,
    ...
    public PaymentFacts paymentFacts() { return new PaymentFacts(publicId, ownerAccountPublicId, responsibleBranchPublicId,
            reservationPublicId, item.publicId(), item.variantPublicId(), item.locationPublicId(), item.quantity(),
            item.skuSnapshot(), item.sizeSnapshot(), status == Status.PENDING_PAYMENT, status == Status.PAID,
            item.totalAmount(), currency, paidAt); }
    public ReceiptFacts receiptFacts() { return new ReceiptFacts(publicId, responsibleBranchPublicId,
    ...
    public record PaymentFacts(UUID orderId, UUID ownerAccountId, UUID responsibleBranchId, UUID reservationId,
            UUID orderItemId, UUID variantId, UUID locationId, long quantity, String sku, String size,
