...
public class PickupPresentationService {
    ...
    public Presentation forOrder(CustomerOrder order) {
        ...
        } else if (fulfillment != null && fulfillment.prepared()) customerStatus = "READY_FOR_PICKUP";
        else if (order.paymentFacts().paid()) customerStatus = "PAID_WAITING_PREPARATION";
        else customerStatus = order.paymentStatus();
        boolean cancellable = order.paymentFacts().paid() && (fulfillment == null || !fulfillment.handedOver());
        return new Presentation(customerStatus, fulfillmentStatus, voidStatus, cancellable);
    }

    public record Presentation(String customerStatus, String fulfillmentStatus, String financialVoidStatus,
