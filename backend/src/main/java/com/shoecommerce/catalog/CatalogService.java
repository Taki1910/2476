...
public class CatalogService {
    private final ProductRepository products; private final ProductVariantRepository variants; private final VariantPriceRepository prices; private final InventoryBalanceRepository balances; private final LocationRepository locations; private final AuthorizationPolicy authorization; private final AuditWriter audit; private final Clock clock;
    public CatalogService(ProductRepository products, ProductVariantRepository variants, VariantPriceRepository prices, InventoryBalanceRepository balances, LocationRepository locations, AuthorizationPolicy authorization, AuditWriter audit, Clock clock) { this.products = products; this.variants = variants; this.prices = prices; this.balances = balances; this.locations = locations; this.authorization = authorization; this.audit = audit; this.clock = clock; }
    ...
    @Transactional public void setStock(SessionPrincipal actor, UUID variantId, UUID locationId, long onHand) { authorization.requirePermission(actor, PermissionCode.INVENTORY_ADJUST); authorization.requireLocationAccess(actor, locationId); ProductVariant variant = variant(variantId); Location location = locations.findByPublicId(locationId).filter(Location::enabled).orElseThrow(() -> new IllegalArgumentException("Location not found or disabled")); InventoryBalance balance = balances.findLockedByVariantAndLocation(variant, location).orElseGet(() -> InventoryBalance.create(variant, location, onHand, clock.instant())); balance.setOnHand(onHand, clock.instant()); balances.save(balance); audit.append(actor, "INVENTORY_SET", "PRODUCT_VARIANT", variant.publicId(), location.branchId(), location.id(), Map.of("onHand", onHand)); }
    @Transactional public void publish(SessionPrincipal actor, UUID variantId) { authorization.requirePermission(actor, PermissionCode.CATALOG_MANAGE); ProductVariant variant = variant(variantId); if (prices.findByVariant(variant).isEmpty()) throw new IllegalStateException("Variant requires a price before publication"); if (!balances.existsByVariantAndOnHandGreaterThan(variant, 0)) throw new IllegalStateException("Variant requires stock before publication"); variant.publish(); audit.append(actor, "VARIANT_PUBLISHED", "PRODUCT_VARIANT", variant.publicId(), null, null, Map.of("sku", variant.sku())); }
