...
export const errorCopy = (error: unknown) => {
  ...
  if (apiError.code === 'AUTH_RATE_LIMITED') return apiError.retryAfterSeconds
    ? t('Too many sign-in attempts. Try again in {seconds} seconds.', { seconds: apiError.retryAfterSeconds })
    : t('Too many sign-in attempts. Please try again later.')
